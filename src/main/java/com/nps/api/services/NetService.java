...
import com.nps.api.models.ozow.OzowPaymentRequest;
...
public class NetService {
    public NetService() {
        LOGGER.info(Emoji.CAT.concat(Emoji.CAT.concat("NetService ready to go ".concat(Emoji.CAT))));
    }
    ...
    public String sendOzowPaymentRequest(OzowPaymentRequest request) throws Exception {
        LOGGER.info(Emoji.PEAR.concat(Emoji.PEAR).concat(Emoji.PEAR).concat("Sending ozow payment request ... ... ... check fields: \uD83D\uDD35 "
                ));

        String hash = hashCheckGenerator.generateOzowHash(request);

        LOGGER.info(Emoji.BLUE_DOT.concat(Emoji.BLUE_DOT).concat("URL for request: ".concat(ozowUrl)));
